...
- [ ] 4. Publish Marketplace — see docs/publishing.md
- [ ] 5. Publish Open VSX — see docs/publishing.md (`npm run publish:ovsx`)
...
3. Never commit secrets, PATs, or Open VSX tokens; do not commit `.vsix` unless asked.
4. Prefer `npm run package` / `npm run publish` / `npm run publish:ovsx` over raw `vsce` / `ovsx` unless debugging.
5. After packaging, report the exact `.vsix` path and version; ask which registry next if unclear.
